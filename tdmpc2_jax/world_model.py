...
class WorldModel(struct.PyTreeNode):
    ...
    def create(
        ...
        *,
        ...
        value_base = partial(
            ...
            [
                NormedLinear(
                    latent_dim, activation=mish, dropout_rate=value_dropout, dtype=dtype
                ),
                NormedLinear(latent_dim, activation=mish, dtype=dtype),
                nn.Dense(num_bins, kernel_init=nn.initializers.zeros),
            ],
    ...
    def reward(
        ...
        logits = self.reward_model.apply_fn({"params": params}, z).astype(jnp.float32)
    ...
    def sample_actions(
        ...
        *,
        ...
        # TODO: Replace with distrax or something that isn't tensorflow.

        # from distrax import MultivariateNormalDiag

        # action_dist = MultivariateNormalDiag(loc=mean, scale_diag=jnp.exp(log_std))
        if deterministic:
            action = mean
        else:
            action = sample_mvn_diag(key, mean=mean, log_std=log_std)
        log_probs = log_prob_mvn_diag(action=action, mean=mean, log_std=log_std)
...
# Generated by Gemini to avoid having to import tensorflow_probability which is buggy asf.


def sample_mvn_diag(
    key: jax.random.PRNGKey, mean: jax.Array, log_std: jax.Array
) -> jax.Array:
    # Standard deviation: sigma = exp(log_std)
    std = jnp.exp(log_std)
    # Generate standard normal noise epsilon ~ N(0, 1)
    eps = jax.random.normal(key, shape=mean.shape)
    # Affine transformation: x = mu + sigma * eps
    return mean + std * eps


def log_prob_mvn_diag(
    action: jax.Array, mean: jax.Array, log_std: jax.Array
) -> jax.Array:
    # Individual variance: sigma^2 = exp(log_std * 2)
    var = jnp.exp(log_std * 2.0)

    # Square error term: (x - mu)^2 / sigma^2
    log_exp_term = -0.5 * jnp.square(action - mean) / var

    # Normalization term: -0.5 * (log(2 * pi) + log(sigma^2))
    # Note: log(sigma^2) is simply 2 * log_std
    log_normalization = -0.5 * (jnp.log(2 * jnp.pi) + 2.0 * log_std)

    # Sum across the event dimension (last axis) to get multivariate log_prob
    return jnp.sum(log_exp_term + log_normalization, axis=-1)
